...
const postAvis = async (req, res) => {
    try {
        ...
        const { outilId, note, difficulte, performance, flexibilite, ...inputAvis } = req.body;

        if (!userId || !outilId) {
            return res.status(400).send({ message: "L'utilisateur et l'outil doivent être spécifiés" });
        }

        if (note < 0 || note > 20 || difficulte < 0 || difficulte > 20 ||
            performance < 0 || performance > 20 || flexibilite < 0 || flexibilite > 20) {
            return res.status(400).send({ message: "Toutes les notes doivent être comprises entre 0 et 20" });
        }
        ...
        await outilModel.findByIdAndUpdate(
            outilId,
...
const putAvisById = async (request, response) => {
    try {
        ...
        if (input.note !== undefined && (input.note < 0 || input.note > 20)) {
            return response.status(400).json({ error: "La note doit être comprise entre 0 et 20" });
        }
        if (input.difficulte !== undefined && (input.difficulte < 0 || input.difficulte > 20)) {
            return response.status(400).json({ error: "La difficulté doit être comprise entre 0 et 20" });
        }
        if (input.performance !== undefined && (input.performance < 0 || input.performance > 20)) {
            return response.status(400).json({ error: "La performance doit être comprise entre 0 et 20" });
        }
        if (input.flexibilite !== undefined && (input.flexibilite < 0 || input.flexibilite > 20)) {
            return response.status(400).json({ error: "La flexibilité doit être comprise entre 0 et 20" });
        }
